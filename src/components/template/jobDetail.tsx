...
const JobDetailPageBuilder = ({ site, jobDetail }: Props) => {
    ...
    return <main contextMenu={name} className={theme?.font.toLocaleLowerCase().replaceAll(' ', '-') ?? 'open-sans'}>
        ...
        <div className='flex justify-center p-[16px] gap-4 border-t-[1px]'>
            ...
            <button className="p-4 rounded-md min-w-[160px] font-semibold" style={{ backgroundColor: GetColorFromTheme('headerBackgroundColor', theme), color: GetHeaderColor(theme) }}>Apply Now</button>
        </div>
